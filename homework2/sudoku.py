...
def group(values, n):
    ...
    """
    stings = len(values) // n
    return [values[i*n: i*n + n] for i in range(stings)]
...
def get_block(values, pos):
    ...
    line = (pos[0] // 3) * 3
    column = (pos[1] // 3) * 3
    return [values[line + i][column + j] for i in range(3) for j in range(3)]
...
def check_solution(solution):
    ...
    for x in range(9):
        for y in range(9):
            row_digits = set('123456789') - set(get_row(solution, (x, y))) # 321
            col_digits = set('123456789') - set(get_col(solution, (x, y))) # 527
