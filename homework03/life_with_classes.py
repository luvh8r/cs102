...
class CellList:
    ...
    def get_neighbours(self, cell):
        return [self.grid[i + cell.row][j + cell.col] for i in range(-1, 2) for j in range(-1, 2) if (i!=0 or j!=0) and 0 <= i + cell.row < self.nrows and 0 <= j + cell.col < self.ncols]
...
if __name__ == '__main__':
    game = GameOfLife(600, 600, 20, 1000)
    game.run()